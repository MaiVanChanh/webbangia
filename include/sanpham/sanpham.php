<?php
	...
	$ma=$_POST['MaLH'];
		...
		$strSQL="SELECT * FROM loai_giay WHERE ma_loai={$ma}";
		$hienthiloai=mysqli_query($conn,$strSQL);
	}
	//hien thi theo gia
	if(isset($_POST['gia']))
	{
		...
		if($gia=='mot')
		{
			$dieukien="WHERE gia < 200000";
			$inloaigiay="Có Giá Dưới 200.000đ";
		}
		else if($gia=='hai')
		{
			$dieukien="WHERE gia >= 200000 and gia <550000";
			$inloaigiay="Có Giá Từ 200.000đ Đến Dưới 550.000đ";
		}
		else if($gia=='ba')
		{
			$dieukien="WHERE gia >= 550000 and gia < 1000000";
			$inloaigiay="Có Giá Từ 550.000đ Đến Dưới 1.000.000đ";
		}
		else if($gia=='bon')
		{
			$dieukien="WHERE gia >= 1000000 and gia < 1500000";
			$inloaigiay="Có Giá Từ 1.000.000đ Đến Dưới 1,500.000đ";
		}
		else if($gia=='nam')
		{
			$dieukien="WHERE gia >= 1500000 and gia < 2000000";
			$inloaigiay="Có Giá Từ 1.500.000đ Đến Dưới 2.000.000đ";
			
		}
		else if($gia=='sau')
		{
			$dieukien="WHERE gia >= 2000000";
			$inloaigiay="Có Giá Trên 2.000.000đ";
		}
	}
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td style="height:25px; background:url(images/trang.jpg) repeat-x; text-align : center; font-size: 22px;" align="center" class="ht" colspan="3"; >	
				...
				Danh Sách Sản Phẩm <?php if($inloaigiay!="") echo $inloaigiay; ?>
		...
		<tr>
			<td>
				<?php 
							while($row=mysqli_fetch_array($giay))
							{
				?>
				<table width="100%" height="120" cellpadding="0" cellspacing="0" border="0" style="border:#CCCCCC 1px solid; margin-top:3px;">
					<tr>
						<td width="150">
							...
							height:103px;">
							<a href="#" onclick="chitiet('<?php echo $row['ma_giay']; ?>')">
							...
							<table width="437" cellpadding="0" cellspacing="0" border="0">
								<tr> 
									<td height="20" align="center">
										...
										Tên Sản Phẩm:</span> 								
										<span class="ten">
										<a href="#" onclick="chitiet('<?php echo $row['ma_giay']; ?>')"><b><?php echo $row									                                         ['ten_giay']; ?></b></a>
										</span>
								...
								<br />
											<a href="#" onclick="chitiet('<?php echo $row['ma_giay']; ?>')">Chi Tiết </a>
											<img src="images/icon.png" border="0" width="14" height="14" align="bottom" />
		<a href="#" onclick="chonhang('<?php echo $row['ma_giay']; ?>','<?php echo $row['ten_giay']; ?>','<?php echo $row['gia']; ?>')">Đặt Mua</a>
									</td>
		...
		<tr>
			<td align="center">
			<table width="100%" height="50" cellpadding="0" cellspacing="0" border="0" style="border:#CCCCCC 1px solid; margin-top:3px;">
					<tr>
						<td>
